...
public class MyBot : IChessBot {
    ...
    int[] PIECE_VALUES = { 1000, 3000, 3000, 5000, 9000, 0 };
	...
	// PROBABLY REMOVE THIS METHOD LATER
	private int getPositionScore(int type, Square square) {
		return 5 * (sbyte)(11111111 & PESTO_TABLES[type, 7 - square.Rank] >> (56 - square.File * 8)); //BUG HERE
    }
    ...
    public Move Think(Board boardOriginal, Timer timerOriginal) {
		...
		int Search(int depth, bool first) {
			if (depth == 0) {
				return Evaluate();
			}
